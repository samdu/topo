...
- **CI reviewer** — Codex in `pr-validate.yaml`, Sol. Runs on every ready PR once the fast jobs are green — the unit, package and script suites — beside the UI tests rather than after them, and answers a different question: *does the code do what the description says, and does the evidence hold?* Its first read of a PR is exhaustive; each later one is given its previous verdict and the change since (the fix loop, below). Automerge merges only when every job in the PR-validate run is green, the reviewer job included, so a Codex outage (its usage limit, most often) means every merge is by hand until it returns.
...
**From round 3 on, only a bug a real user would hit blocks.** `scripts/review-prompt.sh` counts the Codex verdicts already on the PR, and from the third review it tells the reviewer to set `blocking` only for a runtime defect that ordinary use of the app reaches — a lost or corrupted record, a split primary or a leaked secret included when ordinary use reaches it. Everything else, new or still open from an earlier round, is reported non-blocking: a claim worded stronger than the code, a test or Proof entry that could be stronger, a defect in CI, the scripts or the harness, a sequence only a debug path or no user can produce. The PM files each of those as an issue and writes a `deferred:` line naming it, and the round is not spent on them. The PM holds the same bar when it judges a verdict from round 3 on: a blocking finding that is not a bug a user would hit is a ruling and an issue, not a fix round.
...
- The plan review does not loop and the code review does, because a plan is cheap to change and an opinion about it is one round's worth of value, while code that does not do what it says is a defect that has to be closed. The loop converges because it is scoped, capped, spent only on Critical and Important, and from round 3 spent only on bugs a user would hit: past round 2 almost every blocking finding was a claim, a proof, the harness or a corner no user reaches.
