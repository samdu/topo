...
- **Parakeet hearing the fixture through the microphone**, `TOPO_DEBUG_EAR=<directory>`: the real ear, loaded from a directory holding the two model directories (`scripts/fetch-ear-models.sh <directory>` fills one from the manifest, verified). The test runner plays `Tests/Fixtures/purple-elephants.wav` ("purple elephants juggle seven lanterns", from `say`, 16 kHz mono) into the host's output from the press through the hold; it holds that the fixture's energy reached the tap and the sink and that Parakeet's transcript has each of the phrase's words.

The lane comes from the test runner's environment, set on the `xcodebuild` command line with the `TEST_RUNNER_` prefix. `TEST_RUNNER_TOPO_UITEST_AUDIO_INPUT=1` declares a host with an audio input, where a hold refused for want of one fails. `TEST_RUNNER_TOPO_UITEST_EAR_MODELS=<directory>` enables the Parakeet test. The PR check sets both: it installs BlackHole (a virtual loopback device, CI tooling only) as the runner Mac's default input and output, so what the test runner plays comes back through the simulator's microphone, and it restores the models from a cache keyed on the manifest.
