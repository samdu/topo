...
Everything it reads is the button's debug-only accessibility value, a JSON `VoiceInput.Report` decoded strictly (unsigned counts, so a malformed or negative one fails the test): `presses` and `sessions`, the last press's `refusal`, the ear's state, the `recogniser` the last press chose, and its `capture`. The capture is counted by `TapMeter` inside the input tap block itself (buffers, frames, rate, RMS), so it is what the input delivered after the engine started rather than that a tap was installed, plus the samples that reached `SampleSink` at the ear's rate and what the release returned as heard. All of it is behind `#if DEBUG`. Every outcome is recorded as an XCTest activity named for its branch, with the report attached, so the result bundle and the log say what ran.

The app is launched signed in with a placeholder token, past the first-run question, and with `TOPO_DEBUG_KEEP_SPOKEN=1`, so whatever a press hears is printed and never sent and the token never reaches the API. Three tests, one per branch of `VoiceInput.begin`, each selected through `TOPO_DEBUG_EAR` (`DebugRun.ear`) and asserted rather than assumed:

- **The on-device branch over a stub**, `TOPO_DEBUG_EAR=stub`: an ear resident without a model, over an engine that returns no words. It holds that the stub is resident before the press, that the hold chose `parakeet`, that buffers reached the tap for most of the hold, and that the sink holds what the tap delivered at 16 kHz.
- **The fallback**, `TOPO_DEBUG_EAR=loading`: an ear whose load never finishes, so the press takes `SFSpeechRecognizer`. Without it the ordinary ear downloads its 540 MB and loads Parakeet in the simulator within seconds on a fast network, and the "fallback" press would be the on-device one. It holds that the hold chose an Apple recogniser and that buffers reached the tap.
...
**A host with no audio input has no microphone to press, and says so with a skip.** The simulator routes the Mac's default input, and buddybox is a Mac mini with none, so there `AVAudioEngine`'s input node reports a dead format and `begin` refuses the press at its own guard (`VoiceInput.noInput`) before any tap is installed. The test holds that the refused hold opened no session and delivered nothing, then ends in `XCTSkip("missing coverage: …")` naming what did not run; without the models directory the Parakeet test skips the same way. The fallback test also skips, naming the gap, when `SFSpeechRecognizer` reports itself unavailable, or when its on-device recogniser cannot initialise (`kLSRErrorDomain` 300) and ends the session before the tap delivers anything, which is what the PR check's hosted simulator does; any other recogniser error fails. On buddybox all three tests therefore skip, having exercised the permission prompts, the recogniser choice, the audio session's record claim, the guard and the teardown. The tap, the sink, the caption loop and recognition from the microphone are the PR check's loopback lane, or a Mac with a microphone.
