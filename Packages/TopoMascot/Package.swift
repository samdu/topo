...
// samdu/experiments `topo-mascot-swift` at df42ed3 (the Swift port of `topo-mascot-engine`'s
// JavaScript at 3ba6246), copied here unchanged with its idle-cycle tests (RestTests.swift); the
// suite holds it to golden frames that JavaScript rendered (Tests/TopoMascotTests/golden.json,
// made by that experiment's `oracle/golden.mjs`). Foundation only; the iOS app draws him on the
// composer's glass.
let package = Package(
    name: "TopoMascot",
