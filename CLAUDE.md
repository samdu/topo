...
- The phone harness is `Packages/TopoTurn`: `MessagesAPI` (one non-streaming `POST /v1/messages` with the OAuth bearer token, the `oauth-2025-04-20` beta and the Claude Code CLI's identity line as the first system block, which is what a subscription token is accepted for; `Transport` is the seam the tests mock), `TurnRunner` (take the primary lease, append the person's turn, call the model with the last 40 turns, append the reply as its child; a failed call leaves the person's turn in the log), and `LocalRecordDatabase` (a `RecordDatabase` in one JSON file with CloudKit's conflict semantics). The model is a setting: Sonnet 5 by default, Opus 5 or Fable 5.1 from the chat menu.
- The iOS client keeps its log in a `LocalRecordDatabase` under Application Support; it carries no iCloud entitlement yet, so `RecordDatabase` is where CloudKit plugs in. With no socket, its lease probe answers no to every holder.
