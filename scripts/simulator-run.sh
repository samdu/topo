...
if [ "$pressmic" = yes ]; then
  ...
  # The test counts the permission prompts a first press raises, so the app's grants go first:
  # a simulator that answered the microphone on an earlier run would raise none.
  xcrun simctl bootstatus "$udid" -b >/dev/null
